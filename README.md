...
**중심 기능: 🔁 정기 노선 (출퇴근 택시 크루)** — 강남·여의도·광화문·판교 등 주요 업무지구·역세권 거점 52곳(`src/hubs.js`)
사이의 노선이 모두 미리 깔려 있습니다. 첫 화면의 추천 노선이나 거점 칩(출발 → 도착)을 고르면 노선 화면(`/r/jamsil/gangnam`)이 열리고,
시간대별 크루에 참여하거나 새 크루(요일·시각)를 만들고, 노선 채팅에서 정확한 탑승·하차 위치를 정합니다.
