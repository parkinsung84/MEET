...
**중심 기능: 🔁 정기 노선 (출퇴근 택시 크루)** — 서울 행정동 427개 사이의 "출발 동 → 도착 동" 노선이 미리 깔려 있어
구·동만 고르면 노선 화면(`/r/출발동코드/도착동코드`)이 열리고, 시간대별 크루에 참여하거나 새 크루(요일·시각)를 만듭니다.
노선·크루는 로그인 없이 보이고 공유 링크(`/r/…`, `/c/:id`)는 카톡 미리보기를 지원합니다. 만남 장소·크루 채팅은 멤버에게만 보입니다.
행정동 자료: 통계청 SGIS(공공누리 제1유형)를 가공한 [vuski/admdongkor](https://github.com/vuski/admdongkor) (CC BY 4.0) → `public/data/seoul-dongs.json`.
