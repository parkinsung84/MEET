...
export function createNotifier(db, { push = null, log = console } = {}) {
  ...
  return {
     ...
     */
    async notify(userIds, { type, title, body, rideId = null, commuteId = null, url: customUrl = null, skipViewers = false, skipRoom = null, store = true }) {
      // 합승방 알림이면 그 방으로, 정기 노선 알림이면 노선 화면으로
      const url = customUrl ?? (rideId ? `/#/rides/${rideId}` : commuteId ? `/#/c/${commuteId}` : '/#/notifications');
