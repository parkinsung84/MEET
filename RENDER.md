...
### 인증 없이 써 보기 (자동)
SENS·본인확인(PASS)·`SHOW_VERIFICATION_CODES=1` 중 **아무것도 설정돼 있지 않으면** 휴대폰 인증 없이 가입하자마자 합승을 만들고 참여할 수 있어요.
SENS 키나 포트원 키를 넣으면 **자동으로 인증이 필수**로 바뀌어요 (그때 인증 안 한 계정은 인증 화면으로 안내돼요).
직접 정하려면 `REQUIRE_VERIFICATION` = `1`(항상 필수) / `0`(항상 없이).
